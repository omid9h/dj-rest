...
class ColleagueStatusReason(models.TextChoices):
    ...
    TERMINATE_BY_COLLEAGUE = "TERMINATE_BY_COLLEAGUE", gt_l(
        "Contract has been terminated by colleague"
    )
    TERMINATE_BY_COMPANY = "TERMINATE_BY_COMPANY", gt_l(
        "Contract has been terminated by company"
    )
...
class ColleagueCustomerMap(BaseModel):
    ...
    colleague = models.ForeignKey(
        Colleague, on_delete=models.CASCADE, blank=False, null=False
    )
